...
// =================================================
// modifyUser
// - note that this gets user details first
// =================================================
scimgateway.modifyUser = async (baseEntity, id, userObj) => {
  const action = 'modifyUser'
  scimgateway.logger.debug(`${pluginName}[${baseEntity}] handling "${action}" id=${id} userObj=${JSON.stringify(userObj)}`)
  ...
  const path = `/People/${id}`
  const body = {
    enabled: userObj.active !== '' ? (userObj.active ? 1 : 0) : undefined,
    name: {
      first: userObj.name?.givenName || undefined,
      last: userObj.name?.familyName || undefined,
      fullName: userObj.name?.formatted || undefined,
      title: userObj.title || undefined
    },
    contact: {
      emailAddress: userObj.emails?.work?.value || userObj.emails?.undefined?.value || undefined,
      phoneNumber: userObj.phoneNumbers?.work?.value || userObj.phoneNumbers?.undefined?.value || undefined
    }
  }
  ...
  try {
    ...
    if (!urlObj && (err.code === 'ECONNREFUSED' || err.code === 'ENOTFOUND')) {
      if (retryCount < config.entity[baseEntity].baseUrls.length) {
        retryCount++
        updateServiceClient(baseEntity, {
