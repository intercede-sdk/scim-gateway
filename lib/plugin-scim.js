...
scimgateway.getUsers = async (baseEntity, getObj, attributes) => {
  ...
  if (getObj.operator) {
    if (getObj.operator === 'eq' && ['id', 'userName', 'externalId'].includes(getObj.attribute)) {
      ...
      if (getObj.attribute === 'id') path = `/People/${getObj.value}?attributes=${attributes.join()}`
...
scimgateway.createUser = async (baseEntity, userObj) => {
  ...
  const body = {
    ...
    name: {
      first: userObj.name?.givenName || null,
      last: userObj.name?.familyName || null,
      fullName: userObj.name?.formatted || null,
      title: userObj.title || ''
    },
    contact: {
