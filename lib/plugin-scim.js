...
// =================================================
// getUsers - this is used both for search and get individual data
// =================================================
  ...
  try {
    ...
    for (let i = 0; i < responseArr.length && i + 1 - getObj.startIndex < getObj.count; ++i) {
      ...
      const retObj = {
        ...
        roles: userObj.roles?.map((r) => ({ value: r.name, type: r.scope })), // roles not returned in search results
...
scimgateway.createUser = async (baseEntity, userObj, ctx) => {
  ...
  const body = {
    ...
    // invalid role error if name or scope are invalid
    roles: userObj.roles?.map((r) => ({ id: r.value, name: r.value, scope: r.type })),
    group: defaultGroup
  }
...
scimgateway.modifyUser = async (baseEntity, id, attrObj, ctx) => {
  ...
  if (attrObj.credProfile?.id) {
    ...
    // and missing array elements from PUT have an operation: 'delete' instruction sent
    const roles = attrObj.roles?.filter((r) => !r.operation).map((r) => ({ id: r.value, name: r.value, scope: r.type }))
